...
const Alert = () => {
    ...
    const handleClose = () => {
        setIsClosing(true);
        setTimeout(() => dispatch(closeAlert()), 300);
    }

    const onConfirm = async() => {
        await dispatch(updateObject({ loading: true }));

        alertData.onConfirm().then(
            (e) => {
                if (e === true) {
                    ...
                    <div className={`${style.cont} ${alertWindowClass}`} style={{ width: 440 }}>
                        ...
                        <div className={style.bottom}>
                            ...
                            {alertData.type === 'confirm' &&
                                <button type="button" className={[style.btn, style.btn_primary].join(' ')} onClick={onConfirm} disabled={alertData.loading}>
                                    {alertData.loading ?
